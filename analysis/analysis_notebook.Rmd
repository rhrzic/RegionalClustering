...
ggplot(map7) + 
  geom_sf(aes(fill = ave_impr), colour = "transparent") +
  geom_sf(fill = "transparent", size = 0.1, data = gisco0) +
  coord_sf(xlim = c(-10, +30), ylim = c(35, 70)) +
  facet_wrap(. ~ time, ncol = 5) +
  scale_fill_viridis() +
...
data_analysis_men_impr <- data_analysis_men %>% 
  ...
  mutate_at(vars(starts_with("M_")), ~(ifelse(is.finite(-log(lag(.)/.)), -log(lag(.)/.), 0))) %>%
  ungroup() %>%
  mutate(ave_impr = rowMeans(select(., starts_with("M_")), na.rm = TRUE)) %>% 
  filter(time > 2002) %>%
  select(geo, time, ave_impr) %>%
  spread(key = time, value = ave_impr) %>%
  mutate(ave_impr = rowSums(select(., starts_with("20")), na.rm = TRUE)) %>% 
  select(geo, ave_impr)
...
data_analysis_women_impr <- data_analysis_women %>% 
  ...
  mutate_at(vars(starts_with("F_")), ~(ifelse(is.finite(-log(lag(.)/.)), -log(lag(.)/.), 0))) %>%
  ungroup() %>%
  mutate(ave_impr = rowMeans(select(., starts_with("F_")), na.rm = T)) %>% 
  filter(time > 2002) %>%
  select(geo, time, ave_impr) %>%
  spread(key = time, value = ave_impr) %>%
  mutate(ave_impr = rowSums(select(., starts_with("20")), na.rm = T)) %>% 
  select(geo, ave_impr)
